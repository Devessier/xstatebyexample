...
image:
  url: "https://stately.ai/registry/machines/830e93a8-8ee2-4fcc-9868-0bb9b12bc5a9.light.png"
  width: 1303
  height: 600
...
This example is a simplified re-implementation of React Toastify with XState. To play with the demo, **hover over the notifications, focus another window and switch to another tab**.
...
The root machine is `notificationCenterMachine`. This state machine doesn't define any *child state*;
there is only the [*root state*](https://stately.ai/docs/parent-states#root-state).
...
which [*spawns*](https://stately.ai/docs/spawn) a new notification actor and prepends the reference to the `notificationRefs` array. The `notificationMachine` is spawned
with its [*input*](https://stately.ai/docs/input), containing the timeout, title, and description properties,
...
<div class={css({ display: "flex", justifyContent: "center", '& img': { maxWidth: "500px", w: "full" } })}>
  ![A user hovers over the notification and it automatically stops](../../assets/notification-center/animation-stops-on-hover.gif)
</div>

Inspired by React Toastify, the timer of the notification is managed by the browser: the end of the animation is waited for with the `animationend` event
and the animation is paused with the `animation-play-state` CSS property.

When the animation of the progress bar ends, it sends an `animation.end` event to the notification's actor. The state machine then goes to the `Done` state
and sends a `notification.closed` event to its parent, as discussed above.
...
Relying on the animation's state is brilliant because it can serve as the single source of truth.
...
Note that `enqueueActions` *enqueues* actions; it doesn't execute them right away. Actions are always declarative and pure with XState.
...
<Tabs labels={['Machine', 'React', 'Visualizer']} client:load>
  ...
  <Fragment slot="3">
    <StatelyVisualizer src="https://stately.ai/registry/editor/embed/ffb84170-4403-4304-889d-3eefadc0465b?machineId=830e93a8-8ee2-4fcc-9868-0bb9b12bc5a9&mode=design" />
  </Fragment>
</Tabs>
